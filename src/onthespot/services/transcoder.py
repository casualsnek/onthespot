...
class TranscoderService:
    ...
    def start(self):
        while True:
            if len(self.__threads) <= self.__config_service.get("max_concurrent_transcoders"):
                job: MediaDownloadJob = self.__queue.get()
                job.transcode(self.__config_service).on_complete(lambda ret: self.__remove_thread(job)).on_error(
